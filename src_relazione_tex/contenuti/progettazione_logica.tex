...
Imbarcazione(\underline{codiceInternazionale}, \underline{id}, bandiera, nomeCapitano, nPostiLetto, nome, pescaggio, larghezza, LOA);

Molo(\underline{id}, occupato, profonditaMinima, larghezza, lunghezza);

Servizio(\underline{nome});

Addetto(\underline{persona}, \underline{servizio, dataInizioContratto}, dataFineContratto);

Cliente (\underline{persona}, cittadinanza, residenza, quantitaSoste, scontoPersonale);

Persona(\underline{CF},dataNascita, nome, cognome);

Prenotazione(\underline{persona, prevArrivo, molo}  prevPartenza);

Sosta (\underline{imbarcazione, molo, arrivo,} partenza);

Allacciamento(\underline{nome, prezzoUnitario, unitaMisura});

Periodo di apertura (\underline{giorno, apertura, chiusura});

Consumo(\underline{cliente, allaciamento, inizio}, fine, quantita);

Fattura(\underline{fattura, scadenza}, pagato); 
